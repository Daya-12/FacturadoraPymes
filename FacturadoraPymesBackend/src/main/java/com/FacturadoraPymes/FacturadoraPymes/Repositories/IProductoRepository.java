...
public interface IProductoRepository extends CrudRepository<Producto, Integer> {
	...
	@Query(value = "select producto.id_producto,producto.nombre_producto,producto.valor_producto,producto.id_categoria,categoria.nombre_categoria,producto.id_empresa,producto.activo FROM producto INNER JOIN categoria ON producto.id_categoria= categoria.id_categoria INNER JOIN empresa ON empresa.id_empresa=producto.id_empresa WHERE producto.id_empresa=:idEmpresa and producto.activo=:activo group by(producto.id_producto) ORDER BY producto.nombre_producto ASC", nativeQuery = true)
